...
const Cart: React.FC<CartProps> = ({ amount, deleted, onDeleteItem, ref }) => {
  return (
    <div
      className="cart absolute top-[75px] right-1/2 translate-x-1/2 bg-white shadow-lg rounded-[12px] w-[95%] h-full max-h-[260px] max-w-sm z-4 flex flex-col"
      ref={ref}
    >
      ...
      <div className="itembox flex-1 flex flex-col justify-center items-center px-5.5 overflow-y-auto">
        {amount > 0 && !deleted ? (
          ...
          <p className="text-center text-darkGrayishBlue">Your cart is empty</p>
        )}
        {amount > 0 && !deleted ? (
